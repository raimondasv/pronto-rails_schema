...
describe Pronto::RailsSchema do
  describe '#run' do
    subject(:run) { described_class.new(patches, nil).run }
    ...
    context 'with migration' do
      ...
      context 'without schema.rb changes' do
        ...
        it 'adds warning message' do
          expect(run.first.msg).to match(/Migration/)
        end

        context 'with line containing the commit_sha of migration file creation commit' do
          it 'adds warning message', :aggregate_failures do
            expect(run.first.line.content).to match(/class ThatMigration/)
            expect(run.first.line.commit_sha).to eq '4618a01a062aa18aeb205b250004acd1468a6867'
          end
        end
